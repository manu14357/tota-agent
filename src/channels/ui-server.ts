...
export class UIChannel extends BaseChannel {
  ...
  /** Optional orchestrator reference so /api/agents can report live agent runs. */
  private orchestrator: { list(): unknown[] } | null = null;
  ...
  /** Inject the orchestrator so the Agents canvas can fetch current state. */
  setOrchestrator(orchestrator: { list(): unknown[] } | null): void {
    this.orchestrator = orchestrator;
  }

  /** Stream a multi-agent lifecycle event to all connected UI clients. */
  emitAgentEvent(event: unknown): void {
    this.broadcast({ type: 'agentEvent', event });
  }
  ...
  private async handleApi(
    ...
    try {
      ...
      // GET /api/agents — current/recent multi-agent orchestrations
      if (url === '/api/agents' && method === 'GET') {
        json(res, 200, this.orchestrator?.list() ?? []);
        return;
      }
