...
const TOOL_LABELS: Record<string, { icon: string; label: string; argKey?: string; argTransform?: (v: string) => string }> = {
  ...
  browser_hover:           { icon: '🖱',  label: 'Hovering over',         argKey: 'selector',        argTransform: truncate(30) },
  browser_select:          { icon: '🔽', label: 'Selecting option',       argKey: 'selector',        argTransform: truncate(30) },
  browser_drag:            { icon: '🖱',  label: 'Dragging',              argKey: 'source_selector', argTransform: truncate(30) },
  browser_scroll_into_view:{ icon: '↕',  label: 'Scrolling to',          argKey: 'selector',        argTransform: truncate(30) },
  browser_get_url:         { icon: '🌐', label: 'Getting page URL' },
  browser_reload:          { icon: '🔄', label: 'Reloading page' },
  browser_evaluate:        { icon: '⚡', label: 'Evaluating script',      argKey: 'script',          argTransform: truncate(30) },
  browser_navigate:        { icon: '🌐', label: 'Navigating',             argKey: 'direction' },
  browser_cookies_get:     { icon: '🍪', label: 'Getting cookies' },
  browser_cookies_set:     { icon: '🍪', label: 'Setting cookie',         argKey: 'name' },
  browser_cookies_clear:   { icon: '🍪', label: 'Clearing cookies' },
  browser_storage_get:     { icon: '💾', label: 'Reading storage',        argKey: 'kind' },
  browser_storage_set:     { icon: '💾', label: 'Writing storage',        argKey: 'key' },
  browser_storage_clear:   { icon: '💾', label: 'Clearing storage',       argKey: 'kind' },
  browser_pdf:             { icon: '📄', label: 'Saving PDF',             argKey: 'filename',        argTransform: truncate(30) },
  browser_set_viewport:    { icon: '🖥',  label: 'Setting viewport',      argKey: 'preset' },
  // ── Computer use ─────────────────────────────────────────────────────────────
...
export function formatToolResult(toolName: string, result: unknown): string {
  ...
  const RESULT_HINTS: Record<string, (s: string) => string> = {
    ...
    browser_select:          (s) => trimFirst(s),
    browser_drag:            () => 'dragged',
    browser_scroll_into_view:(s) => trimFirst(s),
    browser_get_url:         (s) => trimFirst(s),
    browser_reload:          () => 'reloaded',
    browser_evaluate:        (s) => trimFirst(s),
    browser_navigate:        (s) => trimFirst(s),
    browser_cookies_get:     (s) => {
      const m = s.match(/Cookies \((\d+)\)/);
      return m ? `${m[1]} cookies` : trimFirst(s);
    },
    browser_cookies_set:     (s) => trimFirst(s),
    browser_cookies_clear:   () => 'cleared',
    browser_storage_get:     (s) => `${s.split('\n').filter(Boolean).length} items`,
    browser_storage_set:     (s) => trimFirst(s),
    browser_storage_clear:   () => 'cleared',
    browser_pdf:             (s) => trimFirst(s),
    browser_set_viewport:    (s) => trimFirst(s),
    // Computer use
