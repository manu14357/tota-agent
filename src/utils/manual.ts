...
export function getManual(): string {
  ...
  const tools = [
    ...
    ['browser_click', 'Click an element (by CSS selector or visible text, optional double-click)', 'selector? or text?, double_click?'],
    ['browser_type', 'Type text into an input (SPA-safe: scrolls into view first)', 'selector, text, clear_first?, press_enter?'],
    ...
    ['browser_scroll_into_view', 'Scroll a specific element into the visible viewport', 'selector'],
    ['browser_hover', 'Hover the mouse pointer over an element', 'selector'],
    ['browser_select', 'Select an option from a <select> dropdown', 'selector, value'],
    ['browser_drag', 'Click and drag from one element to another', 'source_selector, target_selector'],
    ['browser_evaluate', 'Run arbitrary JavaScript in the page context', 'script'],
    ['browser_navigate', 'Navigate browser history forward or back', 'direction (back/forward)'],
    ['browser_get_url', 'Get the URL of the currently open page', '—'],
    ['browser_reload', 'Reload the current page', '—'],
    ['browser_close', 'Close the browser session', '—'],
    ['browser_engine', 'Switch browser engine: chromium (default), firefox, or webkit', 'engine'],
    ['browser_cookies_get', 'Get all cookies from the current browser session', '—'],
    ['browser_cookies_set', 'Set a cookie in the current browser session', 'name, value, domain?, path?, expires?, httpOnly?, secure?, sameSite?'],
    ['browser_cookies_clear', 'Clear all cookies from the current browser session', '—'],
    ['browser_storage_get', 'Read items from localStorage or sessionStorage', 'kind (local/session), key?'],
    ['browser_storage_set', 'Write an item to localStorage or sessionStorage', 'kind (local/session), key, value'],
    ['browser_storage_clear', 'Clear all items from localStorage or sessionStorage', 'kind (local/session)'],
    ['browser_pdf', 'Save the current page as a PDF file (Chromium only)', 'filename?, full_page?, landscape?'],
    ['browser_set_viewport', 'Set the browser viewport size or apply a device preset', 'width, height, preset? (desktop/laptop/tablet/mobile)'],
