...
describe('buildModelCatalog', () => {
  ...
  it('prefers qwen3.5:2b for Ollama Local when available', () => {
    const catalog = buildModelCatalog('ollamaLocal', [
      'qwen3.5:2b',
      'llama3.2:latest',
      'qwen3:8b',
    ]);

    expect(catalog.recommendedModel).toBe('qwen3.5:2b');
    expect(catalog.models).toContain('llama3.2:latest');
    expect(catalog.models).toContain('qwen3:8b');
  });

  it('prefers qwen3:8b for Ollama Local when qwen3.5:2b is not available', () => {
    const catalog = buildModelCatalog('ollamaLocal', [
      'qwen3:8b',
      'llama3.2:latest',
      'qwen3:4b',
    ...
    expect(catalog.recommendedModel).toBe('qwen3:8b');
    expect(catalog.models).toContain('qwen3:4b');
  });
