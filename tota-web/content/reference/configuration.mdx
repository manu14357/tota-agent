...
4. Run `calendar_auth` in tota — it prints an auth URL. Visit it, approve access, paste the code back.
5. Tokens are saved to `~/.tota/calendar-token.json` and auto-refreshed.

## Voice (TTS / STT)

| Env Var | Description | Default |
|---------|-------------|---------|
| `OPENAI_API_KEY` | Used for TTS-1 and Whisper STT | — |
...
When `OPENAI_API_KEY` is set, Telegram voice messages are **automatically transcribed** using Whisper before being passed to the agent.
